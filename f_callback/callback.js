...
function sayHello(name, callback) {
    ...
    callback(words); //callback 함수 호출 -> console.log(words)
}
...
const add = (number1, number2, callback) => {
    if (callback) {
        callback(number1 + number2); //callback = console.log(result) -> console.log(1+2)
    }
...
const input = (first, last, callback) => {
    if (callback) {
        callback(first + last + "님"); //calback = console.log(result) -> console.log(first + last + 님)
    }
    // return first + last + "님";
};
...
function makeFullName(성, 이름, callback) {
    if (callback) {
        callback(성 + 이름);
    }
}
...
const money = (price, status, callback) => {
    if (callback) {
